...
# ------------------------------------------------------------
# 1. DATA PROFILING & HELPER FUNCTIONS
# ------------------------------------------------------------
def analyze_dataframe(df: pd.DataFrame) -> Dict:
    if df is None:
        return {'rows':0,'cols':0,'missing_total':0,'missing_by_column':{},'duplicates':0,'memory_mb':0,
...
def get_column_stats_structured(df: pd.DataFrame, col: str) -> str:
    ...
    data = df[col]
    stats = {"Column":col,"Type":str(data.dtype),"Non‑null":f"{data.count()}/{len(df)}","Null":data.isnull().sum(),"Unique":data.nunique()}
    if col in df.select_dtypes(include=['number']).columns:
        stats["Mean"]=f"{data.mean():.2f}"; stats["Std"]=f"{data.std():.2f}"; stats["Min"]=f"{data.min():.2f}"
        stats["25%"]=f"{data.quantile(0.25):.2f}"; stats["50%"]=f"{data.median():.2f}"; stats["75%"]=f"{data.quantile(0.75):.2f}"; stats["Max"]=f"{data.max():.2f}"
    elif col in df.select_dtypes(include=['bool']).columns:
        true_cnt = data.sum(); stats["True"]=true_cnt; stats["False"]=len(df)-true_cnt
    elif col in df.select_dtypes(include=['object']).columns:
        top5 = data.value_counts().head(5); stats["Top 5"]=", ".join(f"{v}({c})" for v,c in top5.items())
    else:
        stats["Sample"]=data.head(5).tolist()
    table = "| Attribute | Value |\n| --- | --- |\n"
    for k,v in stats.items(): table += f"| {k} | {v} |\n"
    return table
...
def detect_outliers_all_numeric(df: pd.DataFrame, skip_ids: bool = True) -> str:
    ...
    if skip_ids: numeric = [c for c in numeric if not is_id_column(c)]
    if not numeric: return "No numeric columns found for outlier detection."
    result = []
    for col in numeric:
...
# ------------------------------------------------------------
# 2. COLUMN OPERATIONS (return new DataFrame)
# ------------------------------------------------------------
...
def change_column_type(df: pd.DataFrame, col: str, new_type: str) -> pd.DataFrame:
    df = df.copy()
    if new_type == 'numeric': df[col] = pd.to_numeric(df[col], errors='coerce')
    elif new_type == 'category': df[col] = df[col].astype('category')
    elif new_type == 'datetime': df[col] = pd.to_datetime(df[col], errors='coerce')
    elif new_type == 'string': df[col] = df[col].astype(str)
    else: raise ValueError(f"Unsupported type: {new_type}")
    return df
...
def merge_two_columns(df: pd.DataFrame, col1: str, col2: str, new_name: str, how: str = 'concat') -> pd.DataFrame:
    ...
    if how == 'concat':
        ...
        else: raise ValueError("Addition works only for numeric columns.")
    else: raise ValueError("how must be 'concat' or 'add'.")
    return df

def create_binned_column(df: pd.DataFrame, col: str, bins: List[float], labels: List[str], new_name: str = None) -> pd.DataFrame:
    df = df.copy()
    if new_name is None: new_name = f"{col}_binned"
...
# ------------------------------------------------------------
# 3. ENCODING FUNCTIONS (return new DataFrame)
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# 4. ALGORITHM SUGGESTIONS (comprehensive)
# ------------------------------------------------------------
def suggest_algorithms(df: pd.DataFrame, target_col: str = None) -> str:
    ...
    if target_col:
        if target_col in numeric_cols:
            ...
            reg_algs = ["Linear Regression","Ridge","Lasso","ElasticNet","Decision Tree Regressor","Random Forest Regressor","Gradient Boosting Regressor","XGBoost Regressor","LightGBM Regressor","CatBoost Regressor","SVR","KNeighbors Regressor","MLP Regressor","AdaBoost Regressor","Bagging Regressor","ExtraTrees Regressor"]
            ...
            class_algs = ["Logistic Regression","Decision Tree Classifier","Random Forest Classifier","Gradient Boosting Classifier","XGBoost Classifier","LightGBM Classifier","CatBoost Classifier","SVC","KNeighbors Classifier","Naive Bayes","MLP Classifier","AdaBoost Classifier","Bagging Classifier","ExtraTrees Classifier","QDA","LDA"]
            suggestion += "\n".join(f"- {alg}" for alg in class_algs) + "\n"
        else:
            suggestion += "**No clear target type. Consider unsupervised learning:**\n- Clustering (KMeans, DBSCAN)\n- Dimensionality reduction (PCA, t-SNE)\n"
    else:
        suggestion += "**No target column. General recommendations:**\n"
        if rows < 100: suggestion += "- Simple models: Linear/Logistic Regression, Decision Trees\n"
        else: suggestion += "- Ensemble methods: Random Forest, XGBoost, LightGBM\n"
        if len(numeric_cols) > 20: suggestion += "- Consider PCA for dimensionality reduction.\n"
        if len(cat_cols) > 10: suggestion += "- Use one‑hot encoding for categorical variables.\n"
    return suggestion
...
**Recommendations:**
"""
    if missing_pct > 5: overview += "- ⚠️ Missing values >5% – consider imputation.\n"
    if duplicate_pct > 5: overview += "- ⚠️ Duplicates >5% – run `df.drop_duplicates()`.\n"
    if numeric_ratio < 30: overview += "- 📊 Low numeric ratio – encode categorical variables for ML.\n"
    return overview
...
def ask_data_analyst(df: pd.DataFrame, profile: Dict, question: str, style: str = "balanced") -> Tuple[str, Optional[plt.Figure]]:
    if df is None: return "Please upload a dataset first.", None
    if profile is None: profile = analyze_dataframe(df)
...
Only output the code block, no extra text."""
    ...
    if row_match:
        ...
        if 1 <= r <= len(df):
            return f"**Answer:** Row {r}\n\n{df.iloc[r-1].to_frame().T.to_markdown()}", None
        else: return f"Row {r} does not exist.", None
    ...
    if "outlier" in q and "code" not in q:
        return f"**Answer:** Outlier detection (IQR method)\n\n{detect_outliers_all_numeric(df, skip_ids=True)}\n\n**Insights:** Outliers can skew statistics.", None
    # ----- Average / mean -----
    if "average" in q or "mean" in q:
        for col in numeric:
            if col in q: return f"**Answer:** Average of **{col}** = {df[col].mean():.2f}", None
        if numeric: return f"**Answer:** Average of first numeric column **{numeric[0]}** = {df[numeric[0]].mean():.2f}", None
        else: return "No numeric columns to compute average.", None
    # ----- Fallback to GPT -----
    styles = {"short":"Provide answer in 1‑2 short sentences with numbers if possible.","balanced":"Provide answer in 2‑3 sentences with explanation.","detailed":"Provide answer in 3‑4 sentences with insights and recommendation."}
    context = f"""DATASET: {len(df)} rows, {len(df.columns)} columns\nFirst 3 rows:\n{df.head(3).to_markdown()}\n\nUser question: {question}\n\n{styles.get(style,styles['balanced'])}\nReturn in format:\n**Answer:** ...\n**Explanation:** ...\n**Insights:** ...\n**Recommendation:** (if applicable)"""
    response = client.chat.completions.create(model=MODEL_NAME, messages=[{"role":"user","content":context}], temperature=0.2, max_tokens=600)
    return response.choices[0].message.content, None

# ------------------------------------------------------------
# 7. VISUALIZATION FUNCTIONS (reliable)
# ------------------------------------------------------------
def create_matplotlib_plot(df, plot_type, x=None, y=None, hue=None):
    fig, ax = plt.subplots(figsize=(10,6))
    fig.patch.set_facecolor('#0f0c29'); ax.set_facecolor('#1a1a3e')
    ...
    try:
        if plot_type == "Histogram":
            if hue:
                ...
                ax.legend()
            else: df[x].hist(bins=30, color='#667eea', edgecolor='white', ax=ax)
            ...
            if y:
                ...
                else: df.groupby(x)[y].mean().plot(kind='bar', color='#667eea', ax=ax); ax.set_ylabel(f'Average {y}')
            else:
                if hue: pd.crosstab(df[x], df[hue]).plot(kind='bar', stacked=True, ax=ax); ax.set_ylabel('Count')
                else: df[x].value_counts().plot(kind='bar', color='#667eea', ax=ax); ax.set_ylabel('Count')
            ax.set_xlabel(x); plt.xticks(rotation=45)
        elif plot_type == "Scatter Plot":
            if hue:
                sc = ax.scatter(df[x], df[y], c=df[hue].astype('category').cat.codes, cmap='viridis', alpha=0.6)
                plt.colorbar(sc, label=hue)
            else: ax.scatter(df[x], df[y], alpha=0.6, color='#667eea')
            ax.set_xlabel(x); ax.set_ylabel(y)
        elif plot_type == "Box Plot":
            if hue: sns.boxplot(data=df, x=x, y=y, hue=hue, palette='Set3', ax=ax)
            else: sns.boxplot(data=df, x=x, y=y, palette='Set3', ax=ax)
            ...
            if hue:
                ...
                ax.legend()
            else: df.sort_values(x).plot(x=x, y=y, kind='line', color='#667eea', marker='o', ax=ax)
            ax.set_xlabel(x); ax.set_ylabel(y)
...
def create_seaborn_plot(df, plot_type, x=None, y=None, hue=None):
    ...
    try:
        if plot_type == "Histogram":
            ...
            if y: sns.barplot(data=df, x=x, y=y, hue=hue, palette='Blues_d', ax=ax); ax.set_ylabel(f'Average {y}')
            else: sns.countplot(data=df, x=x, hue=hue, palette='Blues_d', ax=ax); ax.set_ylabel('Count')
            ax.set_xlabel(x); plt.xticks(rotation=45)
        elif plot_type == "Scatter Plot":
...
def create_plotly_plot(df, plot_type, x=None, y=None, color=None):
    try:
        if plot_type == "Scatter Plot": fig = px.scatter(df, x=x, y=y, color=color, title="Scatter Plot", template="plotly_dark")
        elif plot_type == "Line Plot": fig = px.line(df, x=x, y=y, color=color, title="Line Plot", template="plotly_dark", markers=True)
        elif plot_type == "Bar Chart":
            if y: fig = px.bar(df, x=x, y=y, color=color, title="Bar Chart", template="plotly_dark", barmode='group')
            else:
                data = df[x].value_counts().reset_index()
                fig = px.bar(data, x=x, y='count', color=color, title="Bar Chart", template="plotly_dark")
        elif plot_type == "Histogram": fig = px.histogram(df, x=x, color=color, nbins=30, title="Histogram", template="plotly_dark")
        elif plot_type == "Box Plot": fig = px.box(df, x=x, y=y, color=color, title="Box Plot", template="plotly_dark")
        elif plot_type == "Heatmap":
            numeric_df = df.select_dtypes(include=['number'])
...
# ------------------------------------------------------------
# 8. STREAMLIT UI (8 tabs, all operations update session state)
# ------------------------------------------------------------
st.markdown("""
<style>
...
<div class="hero-section">
    ...
    <div class="hero-description">🧞‍♂️ Hi, I'm Data Genie! Ask me anything – I'll answer, show rows, draw graphs, give you code, rename columns, encode, and engineer features.</div>
    <div class="badge">⚡ Powered by GPT-4o Mini</div>
</div>
""", unsafe_allow_html=True)

# Session state
if "_df" not in st.session_state: st.session_state._df = None
if "_profile" not in st.session_state: st.session_state._profile = None
if "_messages" not in st.session_state: st.session_state._messages = []
...
def load_data(uploaded_file):
    ...
    elif uploaded_file.name.endswith(('.xlsx','.xls')): return pd.read_excel(uploaded_file)
...
if st.session_state._df is not None:
    ...
    if profile is None: profile = analyze_dataframe(df); st.session_state._profile = profile
    ...
    with tabs[0]:
        ...
        for msg in st.session_state._messages[-30:]:
            if msg['role']=='user': st.markdown(f'<div class="message-user">👤 {msg["content"]}</div>', unsafe_allow_html=True)
            else:
                content = msg["content"]
                ...
                if len(parts) > 1:
                    for i in range(0,len(parts),2):
                        if parts[i].strip(): st.markdown(f'<div class="message-assistant">🤖 {parts[i]}</div>', unsafe_allow_html=True)
                        if i+1 < len(parts):
                            code = parts[i+1].strip()
                            col_code, col_btn = st.columns([10,1])
                            with col_code: st.code(code, language='python')
                            with col_btn:
                ...
                else: st.markdown(f'<div class="message-assistant">🤖 {content}</div>', unsafe_allow_html=True)

        st.markdown('<div class="chat-input-fixed">', unsafe_allow_html=True)
        with st.form("chat_form", clear_on_submit=True):
            col1,col2 = st.columns([5,1])
            with col1: user_q = st.text_input("", placeholder="Ask about your data...", label_visibility="collapsed")
            with col2: sent = st.form_submit_button("Send", use_container_width=True)
            if sent and user_q:
    ...
    # ----- Tab 1: Visualize (similar to previous, uses create_*_plot) -----
    with tabs[1]:
        st.markdown("### 📊 Visualizations")
        lib = st.radio("Library",["Matplotlib","Seaborn","Plotly"], horizontal=True)
        if lib=="Matplotlib": plot_types = ["Histogram","Bar Chart","Scatter Plot","Box Plot","Line Plot","Pie Chart"]
        elif lib=="Seaborn": plot_types = ["Histogram","Bar Plot","Scatter Plot","Box Plot","Heatmap"]
        else: plot_types = ["Scatter Plot","Line Plot","Bar Chart","Histogram","Box Plot","Heatmap"]
        ...
        if plot_type in ["Histogram"]:
            ...
            if lib!="Plotly" and cat: hue_col = st.selectbox("Hue (optional)", ["None"]+cat); hue_col = None if hue_col=="None" else hue_col
        elif plot_type in ["Bar Chart","Bar Plot","Pie Chart"]:
            x_col = st.selectbox("Category", cat if cat else df.columns)
            if plot_type in ["Bar Chart","Bar Plot"] and numeric: y_col = st.selectbox("Value (optional)", ["None"]+numeric); y_col = None if y_col=="None" else y_col
            if lib!="Plotly" and plot_type!="Pie Chart" and cat: hue_col = st.selectbox("Hue (optional)", ["None"]+cat); hue_col = None if hue_col=="None" else hue_col
            ...
            if lib!="Plotly" and cat: hue_col = st.selectbox("Hue (optional)", ["None"]+cat); hue_col = None if hue_col=="None" else hue_col
        elif plot_type=="Heatmap": pass
        if st.button("Generate", use_container_width=True):
            if x_col or plot_type=="Heatmap":
                with st.spinner("Drawing..."):
                    try:
                        if lib=="Matplotlib": fig = create_matplotlib_plot(df, plot_type, x_col, y_col, hue_col)
                        elif lib=="Seaborn": fig = create_seaborn_plot(df, plot_type, x_col, y_col, hue_col)
                        else: fig = create_plotly_plot(df, plot_type, x_col, y_col, hue_col)
                        if fig:
                            if lib=="Plotly": st.plotly_chart(fig, use_container_width=True)
                            else: st.pyplot(fig); plt.close()
    ...
    with tabs[2]:
        ...
        if col_cat=="Numeric Columns": cols = profile['numeric_cols']
        elif col_cat=="Categorical Columns": cols = profile['categorical_cols']
        elif col_cat=="Boolean Columns": cols = profile['boolean_cols']
        else: cols = profile['other_cols']
        if cols:
            sel = st.selectbox("Select column", cols)
            if sel: st.markdown(get_column_stats_structured(df, sel))
        ...
        if st.button("Apply Rename"):
            if new_name and new_name != col_to_rename:
                if new_name in df.columns:
                    ...
                    st.success(f"Renamed '{col_to_rename}' to '{new_name}'")
                    st.rerun()
            else: st.warning("Please enter a new name.")
        ...
        if cat_cols_enc:
            ...
            if encoding_type == "Ordinal Encoding":
                ...
                if st.button(f"Apply {encoding_type}"):
                    try:
                        if encoding_type == "Label Encoding": df = apply_label_encoding(df, col_to_encode)
                        else: df = apply_one_hot_encoding(df, col_to_encode)
                        st.session_state._df = df
                        st.session_state._profile = analyze_dataframe(df)
                        st.success(f"{encoding_type} applied. New columns created.")
                        st.rerun()
                    except Exception as e: st.error(f"Error: {e}")
        else: st.info("No categorical columns available for encoding.")
    ...
    with tabs[5]:
        ...
        st.markdown(suggest_algorithms(df, target))
    ...
    with tabs[6]:
        ...
        if feat_type == "Create Binned Column":
            ...
            if numeric_cols:
                ...
                if len(labels) != n_bins: st.warning(f"Number of labels must equal number of bins ({n_bins}).")
                else:
            ...
            else: st.info("No numeric columns available for binning.")
            ...
            if st.button("Create Column"):
                if expression and new_col_name:
                    try:
                        ...
                        st.rerun()
                    except Exception as e: st.error(f"Invalid expression: {e}")
                else: st.warning("Please provide expression and column name.")

    # ----- Tab 7: Download (with custom name) -----
    with tabs[7]:
        st.markdown("### 💾 Download Processed Data")
        default_name = "processed_data.csv"
        custom_name = st.text_input("File name (without .csv)", value=default_name.replace(".csv",""))
        final_name = custom_name.strip() + ".csv" if custom_name.strip() else default_name
        st.download_button(label="📥 Download CSV", data=df.to_csv(index=False).encode('utf-8'), file_name=final_name, mime="text/csv", use_container_width=True)
